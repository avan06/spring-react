...
public class BsLoginCB extends AbstractConditionBean {
     ...
     * @return this. (NotNull)
     */
    public LoginCB acceptPK(Integer id) {
     ...
     * @return this. (NotNull)
     */
    public LoginCB acceptUniqueOf(String loginId, Integer delFlag) {
        assertObjectNotNull("loginId", loginId);assertObjectNotNull("delFlag", delFlag);
        BsLoginCB cb = this;
        cb.query().setLoginId_Equal(loginId);cb.query().setDelFlag_Equal(delFlag);
        return (LoginCB)this;
    }
    ...
    public LoginCQ xdfgetConditionQuery() { // public for parameter comment and internal
        return doGetConditionQuery();
    }

    protected LoginCQ doGetConditionQuery() {
     ...
     */
    public void union(UnionQuery<LoginCB> unionCBLambda) {
        final LoginCB cb = new LoginCB(); cb.xsetupForUnion(this); xsyncUQ(cb); 
     ...
     */
    public void unionAll(UnionQuery<LoginCB> unionCBLambda) {
        final LoginCB cb = new LoginCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
    ...
    public static class HpSpecification extends HpAbstractSpecification<LoginCQ> {
         ...
         * {select max(FOO) from session where ...) as FOO_MAX} <br>
         * session by login_id, named 'sessionList'.
         * <pre>
         * cb.specify().<span style="color: #CC4747">derived${relationMethodIdentityName}()</span>.<span style="color: #CC4747">max</span>(sessionCB <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
         *     sessionCB.specify().<span style="color: #CC4747">column...</span> <span style="color: #3F7E5E">// derived column by function</span>
         *     sessionCB.query().set... <span style="color: #3F7E5E">// referrer condition</span>
         * }, Session.<span style="color: #CC4747">ALIAS_foo...</span>);
         * </pre>
         * @return The object to set up a function for referrer table. (NotNull)
         */
        public HpSDRFunction<SessionCB, LoginCQ> derivedSession() {
            assertDerived("sessionList"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<SessionCB> sq, LoginCQ cq, String al, DerivedReferrerOption op)
                    -> cq.xsderiveSessionList(fn, sq, al, op), _dbmetaProvider);
        }
        /**
        ...
        public HpSDRFunction<LoginCB, LoginCQ> myselfDerived() {
            ...
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<LoginCB> sq, LoginCQ cq, String al, DerivedReferrerOption op)
     ...
     */
    public HpColQyOperand<LoginCB> columnQuery(final SpecifyQuery<LoginCB> colCBLambda) {
     ...
     */
    public void orScopeQuery(OrQuery<LoginCB> orCBLambda) {
        xorSQ((LoginCB)this, orCBLambda);
    }
     ...
     */
    public void orScopeQueryAndPart(AndQuery<LoginCB> andCBLambda) {
        xorSQAP((LoginCB)this, andCBLambda);
    }
    ...
    protected String xgetConditionBeanClassNameInternally() { return LoginCB.class.getName(); }
    protected String xgetConditionQueryClassNameInternally() { return LoginCQ.class.getName(); }
