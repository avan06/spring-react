...
wObj.Application = React.createClass({
	...
	getStateFromFlux: function() {
		...
		var pageStore = wObj.flux.store("PAGE");
		var commonStore = wObj.flux.store("COMMON");
		return {
	...
	render: function() {
		...
		return (
			...
			<ReactBootstrap.Row className="darkBgLarge" style={{margin:0,height:40,lineHeight:"40px",verticalAlign: "middle"}}>
				...
				<ReactBootstrap.Button bsSize="small" bsStyle="primary" onClick={wObj.handleClick} name="btnLogin" style={{marginTop:5}}>
			...
			<wObj.LoginModal isShow={this.state.loginForm_isShow} loginId={this.state.loginForm.loginId}
			 password={this.state.loginForm.password} 
				onClick={wObj.handleClick} onChange={wObj.handleChange} onKeyPress={wObj.handleLoginKeyPress}/>
			<base.Alert isShow={this.state.common.alert.isShow} message={this.state.common.alert.message} onClick={wObj.handleClick}
				/>
			</div>
		);
	},
